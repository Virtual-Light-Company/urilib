...
/**
 * A LDAP protocol handler.
 * <p/>
 * <p/>
 * The basic connection handler for dealing with protcol of the type <CODE>ldap:</CODE> that
 * contains inlined data information.
 * <p/>
 * <p/>
 * A ldap protocol handler is of the format: <PRE> ldap://host[:port]/[distinguished name] </PRE>
 * <p/>
 * The present implementation uses what seems to be logical rather than based on any particular set
 * standard (Couldn't find one).
 * <p/>
 * <p/>
 * If the media type is known then the appropriate content handler is fetched and used to intepret
 * the stream from the LDAPResourceConnection.
 * <p/>
 * <p/>
...
public class Handler extends URIResourceStream
{
        ...
        throws IOException, IllegalArgumentException
    {
        ...
        try
        {
            res = new LDAPResourceConnection(host, port, path);
        }

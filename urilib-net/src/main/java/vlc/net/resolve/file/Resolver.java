 ...
 * <p/>
 * <p/>
 * The bindings file can be specified in one of two forms. It is located in the following manner.
 * <OL> <LI>The system property <CODE>urn.bindings.path</CODE> points to a URL that is the path to
 * the bindings file. To this path the filename <CODE>urn_bindings</CODE> is added. A trailing '/'
 * is added if needed to the base URL. <LI>The file <CODE>urn_bindings</CODE> is searched for in the
 * system classpath. </OL>
 * <p/>
 * If the file is not found after all this, the resolver exits with an exception.
 * <p/>
 * <p/>
 * Currently, there are some restrictions on usage. The bindings file is only loaded on first
 * reference to this class. That means the system property needs to be set before asking to URN
 * resolution and can't be changed.
 * <p/>
 * <p/>
    ...
    implements URNResolverService
{
    ...
    /**
     * Static initialiser reads in the bindings file for use within this class. It does not do any
     * parsing of individual elements at this stage.
     */
    public void init()
    ...
    /**
     * Check that the list of services that this resolver provides matches the requested one.
     *

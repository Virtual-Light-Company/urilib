 ...
 * The specification off a THTTP based RDS implementation may be found at <A
...
public class NamespaceResolver
{
    ...
    /**
     * Add a resource to the namespace ID. Resources are added in order of preference starting with
     * the highest preference first. If the services list is null it is assumed that all services are
     * provided by that host.
     * <p/>
     * Any invalid services in the string are ignored, but does not effect the registering of this
     * url, unless there are no valid services.
     *
    ...
    /**
     * Parse the services string and return the list of services that these describe. The ints are the
     * values of the constants defined in the URIResovlerService interface. Assumes the services list
     * contains at least one service definition. If there is a definition that we do not understand
    ...
    /** Indicate that the startup routines have completed and to deal with the optomisation issues. */
    void setupFinished()
    {
        if(setup_done)
            return;
     ...
     * @throws UnknownNIDException The urn's namespace does not match the one assigned for this
     * class.
     * @throws UnresolvableURIException The type supplied was invalid
     */

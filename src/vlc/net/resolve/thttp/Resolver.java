 ...
 * <P>
 *
 * The THTTP specification is defined by
 * <A HREF="http://www.csl.sony.co.jp/rfc/mirror/rfc2169.txt">RFC 2169</A>
 * and uses a service based on the HTTP specification. Therefore, in order for
 * this resolver to work, it must first be able to get hold of a HTTP resource
 * handler.
 * <P>
 *
 * The resolver requires the use of any named web server so we need to specify
 * the names of the servers that can be set. There are two ways of specifying
 * the servers to be used:
 * <OL>
 * <LI>The system property <CODE>urn.resolver.thttp.path</CODE> points to a URL
 *     that is the path to the bindings file. To this path the filename
 *     <CODE>thttp_urn.conf</CODE> is added. A trailing '/' is added if needed
 *     to the base URL.
 * <LI>The file <CODE>thttp_urn.conf</CODE> is read from the classpath and
 *     parsed to find the location of the servers. The list is a whitespace
 *     separated list of servers and ports following the same syntax as above
 *     for the system property.
 * <LI>The system property <CODE>urn.resolver.thttp.servers</CODE> contains a
 *     pipe '|' separated list of web servers and ports to be queried in order
 *     of preference. Since the web server may be running on a non standard
 *     port, the strings may include an optional port number as such:
 *     <PRE>
 *       www.bar.com:8000 | www.urn.net | resolver.foo.com:4321
 *     </PRE>
 *     These servers are to be considered as generally authoritative for when
 *     they are queried. They should be able to resolve any URN or return the
  ...
  implements URNResolverService
{
  ...
  /**
   * Global initialiser reads in the bindings file for use within this
   * class. It does not do any parsing of individual elements at this stage.
   * This method should only ever be called once as calling it again would
   * trash any previously loaded data
   *
   * @throws ConfigErrorException There was something wrong with the files
   *   that were read in for basic configuration information.
   */
  public void init()
   ...
   * requested one. The only one that this can't handle is II
   *
  ...
  /**
   * Deal with the response of the HTTP connection and create an object for it
   * for an I2L request
   *
   * @param res The connection to the HTTP resource - unconnected
   * @return The object representing the service type
   * @exception IOException I/O problems or invalid response
   * @exception MalformedURLException An error forming the response as a URL
   */
  ...
  /**
   * Deal with the response of the HTTP connection and create an object for it
   * for an I2R request. This also follows redirects if they are needed. It
   * does not directly fetch the object, rather it returns the correctly setup
   * unconnected resource connection for the user to make use of
   *
   * @param res The connection to the HTTP resource - unconnected
   * @return The object representing the service type
   * @exception IOException I/O problems or invalid response

 ...
 * <P>
 *
 * Implements the connection as a standard byte input stream. The data protocol
 * may include header information as part of the parameter information. If
 * the charset is given, then that is set as the encoding type.
 * <P>
 *
 ...
 * <A HREF="http://info.internet.isi.edu/in-notes/rfc/files/rfc2397.txt>RFC 2397</A>.
 * <P>
 *
 ...
 * <P>
 *
 * Because multiple parameters may be put into the stream, we place each
 * parameter into the header info. That way we can use getHeaderField to
 * return all of the details about it. The index of these headers are just as
 * they are declared in the path. If a parameter is <CODE>charset</CODE>
 * that is set to be the encoding type for this connection too. If the
 * <CODE>base64</CODE> parameter is available then this is set as the encoding
 * type in preference. This is a string value.
 * <P>
 *
...
public class DataResourceConnection extends ResourceConnection
{
  ...
  /**
   * The character set special parameter. If this is found in the parameter
   * list of the path then it is used to set the encoding type.
   */
  ...
  */
  ...
  /**
   * Fetch the nth header field. Returns the full length field - name
   * and value with the '=' included.
   *
  ...
  /**
   * Fetch a header field with the given string name. The name is treated
   * using capitalisation. The return value is just the value defined for
   * that header, not the entire string.
   *
  ...
  /**
   * Parse the content to break it into the appropriate pieces.
   * The start of the acutal data is represented by a comma
   *

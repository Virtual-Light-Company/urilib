 ...
 * files do not support output streams.
 * <P>
 *
 * There is an interesting trade off in this situation about when the
 * connection should really take place. There are two steps in the process:
 * the connection to the real underlying JAR file, and the connection to the
 * entry in the JAR file (if you need to). Therefore, every time you want
 * to fetch a JAR file from a JAR URL, there are going to be two instances of
 * this class - one to the real jar file, and then one to the resource
 * representing the JAR URL.
 *
...
public class JarConnection extends JarResourceConnection
{
   ...
   * use. It sets the <CODE>jarFileResource</CODE> protected variable in the
   * base class.
   *
